...
//namespace here is help to bring another varibles which are alr declared to use here under the same namespace
...
//---------------------------------------------------------
//int means returning the int min value
public int findMin()
        {   // we assigned the new var(pCurrent) to the pRoot(1st Node of the tree)
            Node pCurrent = pRoot;
            //(keep looping like if pCurrent which in pLeftChild
            //here means if there is a leftchild of pCurrent then keep 
            while(pCurrent.pLeftChild !=null)
                //if there is one while loop statement no need to put curly braces
                //...keep assign leftchild node to the current 
                pCurrent = pCurrent.pLeftChild;
            //it returns when there is no more leftchild so iData is the Minimum one
